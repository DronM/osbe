...
(function ($) {
    $.fn.EasyTree = function (options) {
        var defaults = {
            ...
            onExpand:null,
            onCollapse:null,
            classOpen:"glyphicon-folder-open",
            classClose:"glyphicon-folder-close",
            classFile:"glyphicon-file",
            expanded:false,
            i18n: {
                ...
                collapseTip: 'Свернуть',
                expandTip: 'Развернуть',
                selectTip: null,
                unselectTip: null,
                editTip: 'editTip',
                addTip: 'addTip',
        ...
        this.each(function () {
            ...
            $.each($(easyTree).find('ul > li'), function() {
                ...
                if($(this).is('li:has(ul)')) {
                    ...
                    $(this).find(' > span > span').addClass(options.expanded? options.classOpen:options.classClose);//'glyphicon-folder-open'
                    $(this).find(' > span > a').text(text);
                    $(this).append(children);
		    var ch_nodes = children.find(' > li');
	    	    ch_nodes.hide('fast');
                }
                else {
                    text = $(this).text();
                    $(this).html('<span><span class="glyphicon"></span><a href="javascript: void(0);"></a> </span>');
                    $(this).find(' > span > span').addClass(options.classFile);
                    $(this).find(' > span > a').text(text);
                }
            });

            $(easyTree).find('li:has(ul)').addClass('parent_li').find(' > span').attr('title', options.expanded? options.i18n.collapseTip:options.i18n.expandTip);
            ...
            if (options.addable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        var item = $('<li><span><span class="glyphicon '+options.classFile+'"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
                        ...
                        if (selected.length <= 0) {
                            ...
                            if ($(selected).hasClass('parent_li')) {
                                ...
                                $(selected).addClass('parent_li').find(' > span > span').addClass(options.classOpen).removeClass(options.classFile);
                    ...
                    if (selected.length <= 0) {
                        ...
                        $(easyTree).find('.alert .alert-content .confirm').on('click', function () {
                            ...
                            if($(selected).parent('ul').find(' > li').length <= 1) {
                                $(selected).parents('li').removeClass('parent_li').find(' > span > span').removeClass(options.classOpen).addClass(options.classFile);
            ...
            $(easyTree).delegate('li.parent_li > span', 'click', function (e) {
                ...
                if (children.is(':visible')) {
                    ...
                    if (options.onCollapse){
                    	options.onCollapse.call(this,e,this.parentNode);
                    }
                } else {
                    children.show('fast');
                    ...
                    if (options.onExpand){
                    	options.onExpand.call(this,e,this.parentNode);
                    }
                }
                e.stopPropagation();
            });
