...
/* constructor */
function WindowModal(options){
	options = options || {};
	WindowModal.superclass.constructor.call(this,options);
}
extend(WindowModal,WindowForm);

WindowModal.prototype.open = function(){
	var features = "width="+this.m_width+","+
				"height="+this.mheight+","+
				"top="+this.m_top+","+
				"left="+this.m_left;
	f_dialogOpen(this.getURL(),this.m_caption,features);
	this.m_WindowForm = window;
}
WindowModal.prototype.close = function(){
	f_dialogClose();
}
WindowModal.prototype.getContentParent = function(){
	return nd("modalWindow");
}

/*
	original code
*/
var N_BASEZINDEX = 9999;
...
function f_dialogOpen (s_url, s_title, s_features) {
	...
	var content;
	if (s_url&&s_url.length){
		content = '<iframe width="'+n_width+'" height="'+n_height+'" src="'+s_url+'"></iframe>';
	}
	else{
		content = '<div id="modalWindow"><div>';
	}
	e_window.innerHTML = 
		...
		((s_title && !isIE()) ? s_title : '') +
		'</span><img src="img/form/close.png" onclick="top.f_dialogClose();" onmousedown="return false;" style="float:right"></th></tr><tr><td>'+
		content+
		'</td></tr></table>';
