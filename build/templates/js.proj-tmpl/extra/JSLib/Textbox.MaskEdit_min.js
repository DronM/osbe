...
$import("Textbox.Common.js");var UPPER="\n";var LOWER="\r";var NUM="\t";function InitializeMaskEdit(ctrl,strMaskPattern)
...
obj.attachEvent('onkeydown',function(){eval('__MaskEditDispatcher(event);');});obj.attachEvent('onkeyup',function(){eval('__SetCursorPosition(event);');});obj.attachEvent('onkeypress',function(){eval('__OnMaskEditUpdated(event);');});obj.attachEvent('onfocus',function(){eval('__OnMaskEditFocus(event);');});obj.setAttribute("position",-1);if(document.all)
{obj.setAttribute("autocomplete","off");obj.onpaste=function(){return false;};obj.ondrag=function(){return false;};obj.ondrop=function(){return false;};obj.oncut=function(){return false;};obj.oncontextmenu=function(){return false;};}}
...
{if(__GetKeyCode(evt)==VKeyCode.VK_LEFT||__GetKeyCode(evt)==VKeyCode.VK_RIGHT||__GetKeyCode(evt)==VKeyCode.VK_NUMLOCK||__GetKeyCode(evt)==VKeyCode.VK_HOME||__GetKeyCode(evt)==VKeyCode.VK_END||__GetKeyCode(evt)==VKeyCode.VK_TAB||__GetKeyCode(evt)==VKeyCode.VK_LEFT)
{return true;}
var obj=evt.srcElement==null?evt.target:evt.srcElement;var strMaskPattern=obj.getAttribute("maskPattern");var nStart=__GetCursorStartPos(evt);var nEnd=__GetCursorEndPos(evt);if(nStart!=nEnd)
