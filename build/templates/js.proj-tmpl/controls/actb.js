...
actbAJX.prototype.setObjectId = function(inputNode){
	...
	while (!id_found && this.m_respModel.getNextRow()){
		id_found = (this.m_respModel.getFieldById(this.m_acValueFieldId).getValue()==inputNode.value);
		if (id_found){
			for (var i=0;i<this.m_acKeyFieldIds.length;i++){
				DOMHandler.setAttr(
...
function actb(obj,winObj,servConnect){
	...
	EventHandler.addEvent(actb_curr,"focus",
			function(){
				...
				//EventHandler.addEvent(actb_curr,"blur",actb_clear,false);
				EventHandler.addEvent(winDocum,"keypress",actb_keypress,false);
				DOMHandler.removeAttr(actb_curr,"incorrect_obj");
			},
			false
		);
	EventHandler.addEvent(actb_curr,"blur",
			function(){
				...
				if (servConnect.m_isObject
					...
					if (!k||!k.length){
						DOMHandler.addAttr(actb_curr,"incorrect_obj","1");
					}
				}
				actb_removedisp();
			},
	...
	function actb_clear(evt){
		...
		*/
		EventHandler.removeEvent(winDocum,"keydown",actb_checkkey,false);
		EventHandler.removeEvent(actb_curr,"blur",actb_clear,false);
		EventHandler.removeEvent(winDocum,"keypress",actb_keypress,false);
		actb_removedisp();
	}
	...
	function actb_generate(){
		if (nd('tat_table',winDocum)){ actb_display = false;winDocum.body.removeChild(nd('tat_table',winDocum)); } 
		if (actb_kwcount == 0){
			actb_display = false;
			return;
		}
